...
* 方差：
	...
	* The asymptotic variance of $\hat{\beta}_1^{I V}$ is $$\widehat{AVar}(\hat{\beta_1}^{I V})=\frac{\hat{\sigma}^2}{S S T_x R_{x, z}^2}$$where $S S T_x=\sum_{i=1}^n\left(x_i-\bar{x}\right)^2$, and $R_{x, z}^2$ is the R-squared of $x_i$ on $z_i$.
...
* 解决办法：
	...
	2. 求偏导的均值
		* Average marginal effect:$$[N^{-1} \sum_{i=1}^N g(\hat{\beta_0}+\boldsymbol{x}_{\boldsymbol{i}} \hat{\boldsymbol{\beta}_{\mathbf{1}}})] \hat{\beta_j}$$
