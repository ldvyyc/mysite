...
* 即$$\lim _{n \rightarrow \infty} P(a<\frac{S_n-E(S_n)}{\sigma_{S_n}} \leq b)=\frac{1}{\sqrt{2 \pi}} \int_a^b e^{-x^2 / 2} d x$$
...
* 随机变量X表示伯努利试验直到第一次成功（等时）的次数，则$$\mathbb{P}(X=k)=p q^{k-1},\ k=1,2,\cdot\cdot\cdot.$$这里p表示单一试验成功的概率。均值、方差、矩母函数如下:$$\begin{array}{l}{{\mu=\frac1p,}}\\\\ {{\sigma^{2}=\frac q{p^{2}},}}\\\\ {{M(t)=\frac{p e^{t}}{1-q e^{t}}.}}\end{array}$$
...
* 现给定两个总体。第一、二个样本分别来自第一、二个总体, 样本容量、统计量分别为 $n_1, n_2$ 和 $S_1, S_2$, 均值和标准差（即偏差）分别为 $\mu_{S_1}, \mu_{S_2}$ 和 $\sigma_{S_1}, \sigma_{S_2}$ 。现考虑所有来自这两个总体样本的所有组合, 可得和、差$$S_1+S_2, S_1-S_2$$的分布。不难知道，若 $S_1, S_2$ 独立，则$$\begin{gathered}
