...
* 随机变量X表示伯努利试验直到第一次成功（等时）的次数，则$$P(X=k)=pq^{k-1},k=1,2,\cdot\cdot\cdot.$$这里p表示单一试验成功的概率。均值、方差、矩母函数如下:$$\begin{array}{l}{{\mu=\frac1p,}}\\\\ {{\sigma^{2}=\frac q{p^{2}},}}\\\\ {{M(t)=\frac{p e^{t}}{1-q e^{t}}.}}\end{array}$$
